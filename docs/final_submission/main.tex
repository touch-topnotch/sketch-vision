...
\paragraph{Pseudocode summary.} The key helper routines (cells 5--56 of \repo{experiments/find\_boundings.ipynb})
are reproduced below for portability:

\begin{verbatim}
read_obj_vertices_faces(path):
    vertices, faces = [], []
    for each line in OBJ file:
        if line starts with "v": append vertex (x, y, z)
        elif line starts with "f": collect face indices (1-indexed)
...
coords_to_pixels(coords, width=1500, height=1500, padding=0.05):
    compute min/max spans, scale to canvas with padding
    center coordinates and flip Y axis for image convention
    return integer pixel array
\end{verbatim}

These helpers feed a loop that renders tri-planar canvases for every OBJ mesh and saves them under \repo{experiments/*.png}.
